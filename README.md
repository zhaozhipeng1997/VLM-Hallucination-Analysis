...
Vision-language models (VLMs) increasingly underpin multimodal world simulation and communication systems---virtual environment generation, digital twin visualization---yet hallucination undermines the fidelity of simulated world representations. To build trustworthy systems, we must understand \emph{why} they hallucinate, not merely \emph{when}. We propose a mechanistic taxonomy decomposing every hallucinated token into one of two failure modes: \textbf{encoding failure}, where visual information is never properly captured, or \textbf{arbitration failure}, where it is encoded but overridden by language priors in upper layers. We introduce gradient-based attribution that localizes the visual dependency signal $\Delta_t$ to individual components along the Encoding Pathway (vision encoder, projection, early layers) and Arbitration Pathway (upper self-attention, MLP, LM head). Counterfactual ablation causally validates the taxonomy: encoding knockout uniformly degrades $\Delta_t$, while arbitration knockout produces selective degradation where language priors conflict with visual evidence (Levene's test $p<0.001$). Benchmarking against counterfactual restoration patching confirms gradient-based attribution recovers the same critical heads as full causal intervention (Spearman $\rho{=}0.531$, $p{<}0.001$, $513{\times}$ speedup; top-20 heads capture $89.6\%$ of causal restoration KL at zero false-positive precision), with encoding heads carrying $12.8{\times}$ more unique visual information per head. Across three architectures, we find (i) arbitration failure dominates in LLaVA-1.5 and Qwen2.5-VL ($85$--$88\%$), (ii) InternVL3.5 exhibits a balanced profile with $39.7\%$ grounded tokens, (iii) Qwen2.5-VL and InternVL3.5 share convergent circuits ($\rho{=}0.887$), and (iv) causal ablation confirms core predictions with pathway effect sizes of $0.28$ (encoding) vs.\ $0.62$ (arbitration). Our results demonstrate that mechanistic failure-mode diagnosis is a foundation for building safer multimodal world simulation and communication systems.

## Method Overview

![Method Overview](asset/paper4.svg)

The Encoding–Arbitration taxonomy decomposes VLM failures into two mechanistic pathways:

- **Encoding Pathway (E):** Vision encoder → Projector → Early LLM layers (1–⌊L/4⌋)
- **Arbitration Pathway (A):** Upper self-attention → MLP → LM head (⌊L/4⌋+1 through L)

For each generated token, a visual dependency score Δt = log PF(chosen) − log PC(chosen) is computed through paired factual/counterfactual forward passes. Per-token classification uses encoding strength (L2 norm of head activations) and arbitration ratio (||hF|| / (||hF|| + ||hC||)) with per-run relative thresholds. Gradient-based component attribution identifies which attention heads carry visual information, validated against counterfactual restoration patching (the causal gold standard).
...
**Data format:**
- Images: RGB JPEG, various resolutions, typical ~640×480
...
Used for Table 3 in the paper (N = 100 COCO images × 5 tasks = 1,500 image–task pairs):
...
### Step 2: Gradient vs. Causal Patching Benchmark (513× Speedup Validation)

Benchmarks gradient-based attribution against counterfactual restoration patching (the causal gold standard). Produces Spearman ρ and top-k coverage metrics.
...
Output: `results/paper4_revision/topk_coverage/topk_coverage.json`

### Step 3: Top-k Causal Coverage Analysis (Spearman ρ = 0.531)
...
```bash
# Run all
python mechanistic_analysis/paper4_revision_experiments.py \
    --experiment all \
    --model llava-1.5 \
    --num_samples 1000

# Or individually:
python mechanistic_analysis/paper4_revision_experiments.py --experiment p0 --model llava-1.5
python mechanistic_analysis/paper4_revision_experiments.py --experiment p1 --model llava-1.5
python mechanistic_analysis/paper4_revision_experiments.py --experiment p2 --model llava-1.5
python mechanistic_analysis/paper4_revision_experiments.py --experiment p3 --model llava-1.5
```
...
| Flag | Experiment | Description |
|---|---|---|
| `r1` | Path integration | Δ signal verification across forward pass |
...
```bash
# Run all
python mechanistic_analysis/paper4_r2_experiments.py \
    --experiment all \
    --model llava-1.5 \
    --num_images 500
```
...
| `e2` | Pathway boundary sensitivity | ±4–6 pp shift across boundary range |
| `e3` | Universal cross-architecture intervention | CHAIRs 2.0% → 0.0% at α=1.2 |
| `e4` | Oracle threshold calibration | τenc: LLaVA=0.997, Qwen=2.141, InternVL=2.703 |
| `e5` | Counterfactual type comparison | Shuffled patches vs. zero pixel_values |

```bash
# Run all
python mechanistic_analysis/paper4_r3_experiments.py \
    --experiment all \
    --model llava-1.5 \
    --num_images 500
```
...
├── paper4_revision/
│   ├── topk_coverage/
│   │   └── topk_coverage.json                  # ρ=0.531, 89.6% coverage
│   ├── p1_vqa/
...
│   └── e4_oracle_threshold_*.json              # τenc calibration
└── multi_task/
...
| Spearman ρ=0.531, 89.6% coverage | Step 2–3 | `paper4_revision/topk_coverage/topk_coverage.json` |
| Levene's F=426.1, p<0.001 | Step 6 (e1) | `paper4_r3/e1_fine_ablation_*.json` |
| CHAIRs 2.0%→0.0% at α=1.2 | Step 6 (e3) | `paper4_r3/e3_universal_intervention_*.json` |
| Oracle τenc calibration | Step 6 (e4) | `paper4_r3/e4_oracle_threshold_*.json` |
| VQA cross-dataset: grd 0.5%→40.5% | Step 4 (p1) | `paper4_revision/p1_vqa/vqa_summary_*.json` |
